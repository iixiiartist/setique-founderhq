...
import { RevenueModule, CashFlowModule, MetricsModule, FinancialsViewSelector, type FinancialsViewType } from './financials';
    ...
    <li className="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg shadow-sm">
        <div className="flex-grow">
            <p className="font-semibold text-gray-900">
                {new Date(item.date + 'T00:00:00').toLocaleDateString(undefined, { timeZone: 'UTC', year: 'numeric', month: 'long', day: 'numeric' })}
            </p>
...
const ExpenseItem: React.FC<{
    ...
    const [editForm, setEditForm] = React.useState<Partial<Omit<Expense, 'id' | 'notes'>>>({
        date: item.date, category: item.category, amount: item.amount,
        description: item.description, vendor: item.vendor, paymentMethod: item.paymentMethod
    });

    const handleSave = () => {
        if (Object.keys(editForm).length > 0) onUpdate(editForm);
        setIsEditing(false);
    };
            ...
            <li className="p-3 bg-amber-50 border border-amber-200 rounded-lg shadow-sm">
                <div className="space-y-2">
                    <input type="text" value={editForm.description || ''} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })}
                        className="w-full p-2 border border-gray-300 rounded-md font-semibold focus:outline-none focus:ring-2 focus:ring-black" placeholder="Description" required />
                    <div className="grid grid-cols-2 gap-2">
                        <input type="number" min={0} step="0.01" value={editForm.amount ?? 0} onChange={(e) => setEditForm({ ...editForm, amount: Math.max(0, Number(e.target.value)) })}
                            className="p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black" placeholder="Amount" required />
                        <input type="date" value={editForm.date || ''} onChange={(e) => setEditForm({ ...editForm, date: e.target.value })}
                            className="p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black" required />
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                        <select value={editForm.category || 'Other'} onChange={(e) => setEditForm({ ...editForm, category: e.target.value as ExpenseCategory })}
                            className="p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black">
                            {EXPENSE_CATEGORY_OPTIONS.map(cat => <option key={cat} value={cat}>{cat}</option>)}
                        </select>
                        <select value={editForm.paymentMethod || ''} onChange={(e) => setEditForm({ ...editForm, paymentMethod: (e.target.value || undefined) as PaymentMethod | undefined })}
                            className="p-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-black">
                            <option value="">Payment Method</option>
                            {PAYMENT_METHOD_OPTIONS.map(pm => <option key={pm} value={pm}>{pm}</option>)}
                        </select>
                    </div>
    ...
    return (
        <li className="flex items-center justify-between p-3 bg-white border-2 border-black shadow-neo">
            ...
            <div className="flex gap-2 shrink-0 ml-4">
                <button onClick={() => setIsEditing(true)} className="text-lg hover:text-blue-600 transition-colors" aria-label={`Edit expense: ${item.description}`}>✎</button>
                <button onClick={onDelete} className="text-xl font-bold hover:text-red-500 transition-colors" aria-label={`Delete expense: ${item.description}`}>&times;</button>
            </div>
        </li>
    );
};
...
const FinancialsTab: React.FC<{
    ...
    const [currentView, setCurrentView] = useState<FinancialsViewType>('overview');
    ...
    const handleLog = (data: FinancialLogFormData) => {
        actions.logFinancials({ date: data.date, mrr: data.mrr, gmv: data.gmv, signups: data.signups });
    };
    ...
    const handleExpense = (data: ExpenseFormData) => {
        actions.createExpense({
            ...
            category: data.category as ExpenseCategory, vendor: data.vendor || '',
    ...
    const filteredExpenses = useMemo(() => {
        ...
        return [...filtered].sort((a, b) => expenseSortBy === 'date'
    ...
    const monthlyExpenses = useMemo(() => expenses.filter(e => e.date.startsWith(currentMonthKey)).reduce((sum, e) => sum + e.amount, 0), [expenses, currentMonthKey]);
    const previousMonthExpenses = useMemo(() => expenses.filter(e => e.date.startsWith(previousMonthKey)).reduce((sum, e) => sum + e.amount, 0), [expenses, previousMonthKey]);
    const monthlyExpenseDelta = monthlyExpenses - previousMonthExpenses;

    const sortedLogs = useMemo(() => [...items].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()), [items]);
    const chronologicalLogs = useMemo(() => [...items].sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime()), [items]);
    const chartData = useMemo(() => chronologicalLogs.map(log => ({ date: log.date, mrr: log.mrr, gmv: log.gmv })), [chronologicalLogs]);
    ...
    const { latestSignups, latestMrr, latestGmv } = {
        latestSignups: latestFinancials?.signups ?? 0,
        latestMrr: latestFinancials?.mrr ?? 0,
        latestGmv: latestFinancials?.gmv ?? 0,
    };

    const snapshotDescription = latestFinancials ? `Latest entry: ${latestLogDateLabel}` : 'Log your first financial snapshot to unlock insights';
    const signupsDescription = latestFinancials ? `Latest entry: ${latestLogDateLabel}` : 'Log your first financial snapshot to track signups';
    const totalExpenseDescription = expenses.length > 0 ? `${expenses.length} logged expense${expenses.length === 1 ? '' : 's'}` : 'Track your first expense';
    const monthlyExpenseDescription = `Spending in ${currentMonthLabel}`;

    const buildTrend = (delta: number | null, isCurrency = false, isExpense = false) => {
        if (delta === null) return undefined;
        return {
            label: `${formatDelta(delta, isCurrency)} vs ${isExpense ? previousMonthLabel : 'prior entry'}`,
            tone: (isExpense ? delta <= 0 : delta >= 0) ? 'positive' : 'negative',
            direction: delta === 0 ? 'flat' : undefined
        } as const;
    };

    return (
        <div className="space-y-6">
            <FinancialsViewSelector currentView={currentView} onViewChange={setCurrentView} />

            {currentView === 'revenue' && data && <RevenueModule data={data} actions={actions} workspaceId={workspaceId} productsServices={productsServices} />}
            {currentView === 'cashflow' && data && <CashFlowModule data={data} />}
            {currentView === 'metrics' && data && <MetricsModule data={data} workspaceId={workspaceId} />}
            {currentView === 'forecasting' && <FinancialForecastingModule />}
            {currentView === 'analytics' && <RevenueAnalyticsDashboard />}
            {currentView === 'marketing' && <MarketingAttributionDashboard />}

            {currentView === 'overview' && (
                <div className="space-y-4">
                    {/* KPIs */}
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-3">
                        <KpiCard title="MRR" value={formatCurrency(latestMrr)} description={snapshotDescription} trend={buildTrend(mrrDelta, true)} />
                        <KpiCard title="GMV" value={formatCurrency(latestGmv)} description={snapshotDescription} trend={buildTrend(gmvDelta, true)} />
                        <KpiCard title="Signups" value={formatNumber(latestSignups)} description={signupsDescription} trend={buildTrend(signupDelta)} />
                        <KpiCard title="Total Expenses" value={formatSpend(totalExpenses)} description={totalExpenseDescription} />
                        <KpiCard title="Monthly Spend" value={formatSpend(monthlyExpenses)} description={monthlyExpenseDescription} trend={buildTrend(monthlyExpenseDelta, true, true)} />
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div className="h-48">
                                    <ResponsiveContainer width="100%" height="100%">
                                        <LineChart data={chartData}>
                                            ...
                                            <XAxis dataKey="date" tick={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 9 }}
                                                tickFormatter={(value: string) => new Date(`${value}T00:00:00Z`).toLocaleDateString(undefined, { timeZone: 'UTC', month: 'short', day: 'numeric' })} />
                                            <YAxis tickFormatter={(value: number) => currencyFormatterNoCents.format(Number(value))}
                                                tick={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 9 }} width={60} />
                                ...
                                {expensesByCategory.length > 0 ? (
                                    <div className="h-48 flex items-center">
                                        <ResponsiveContainer width="60%" height="100%">
                                            <PieChart>
                                                <Pie data={expensesByCategory} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={60} label={false}>
                                                    {expensesByCategory.map((_, index) => <Cell key={`cell-${index}`} fill={EXPENSE_COLORS[index % EXPENSE_COLORS.length]} />)}
                                                </Pie>
                            ...
                            <div className="bg-white p-4 border-2 border-black shadow-neo">
                                ...
                                {sortedLogs.length > 5 && <p className="text-xs text-gray-500 mt-2 text-center">+ {sortedLogs.length - 5} more entries</p>}
                            </div>
                            ...
                            <div className="bg-white p-4 border-2 border-black shadow-neo">
                                ...
                                {filteredExpenses.length > 5 && <p className="text-xs text-gray-500 mt-2 text-center">+ {filteredExpenses.length - 5} more entries</p>}
                            </div>
                        </div>
                        ...
                        <div className="space-y-4">
                                    ...
                                    {({ watch, formState }) => {
                                        ...
                                        return (
                                            <>
                                                ...
                                                <div className="grid grid-cols-2 gap-2">
                                                    <FormField name="mrr" label="MRR ($)" type="number" min={0} placeholder="1000" required />
                                                    <FormField name="gmv" label="GMV ($)" type="number" min={0} placeholder="10000" required />
                                                </div>
                                                <FormField name="signups" label="New Signups" type="number" min={0} step={1} placeholder="100" required />
                                                <div className="flex justify-between items-center pt-1">
                                                    <span className="font-mono text-xs font-semibold text-green-600">+{calculatedXp} XP</span>
                                                    <Button type="submit" size="sm" loading={formState.isSubmitting}>Log</Button>
                                                </div>
                                            </>
                                        );
                                    }}
                            ...
                            <div className="bg-white p-4 border-2 border-black shadow-neo">
                                ...
                                <Form schema={expenseSchema}
                                    ...
                                    onSubmit={handleExpense} className="space-y-2">
                                    {({ formState }) => (
                                        <>
                                            <div className="grid grid-cols-2 gap-2">
                                                <FormField name="date" label="Date" type="date" required />
                                                <FormField name="amount" label="Amount ($)" type="number" step="0.01" min={0} placeholder="0.00" required />
                                            </div>
                                            <FormField name="description" label="Description" type="text" placeholder="AWS hosting" required />
                                            <div className="grid grid-cols-2 gap-2">
                                                <SelectField name="category" label="Category" required options={EXPENSE_CATEGORY_OPTIONS.map(cat => ({ value: cat, label: cat }))} />
                                                <FormField name="vendor" label="Vendor" type="text" placeholder="Optional" />
                                            </div>
                                            <SelectField name="paymentMethod" label="Payment Method"
                                                options={[{ value: '', label: 'Select' }, ...PAYMENT_METHOD_OPTIONS.map(method => ({ value: method, label: method }))]} />
                                            <div className="flex justify-end pt-1">
                                                <Button type="submit" variant="danger" size="sm" loading={formState.isSubmitting}>Log Expense</Button>
                                            </div>
                                        </>
                                    )}
                                </Form>
