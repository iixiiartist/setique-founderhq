...
const QUICK_SUGGESTIONS = [
  { label: '=ƒôï Executive Summary', prompt: 'Write an executive summary for this document based on our business context' },
  { label: '=ƒÄ» Key Messages', prompt: 'Generate 3-5 key messages that align with our value proposition and target market' },
  { label: '=ƒæÑ Target Audience', prompt: 'Describe our target audience with pain points and buying behaviors' },
  { label: '=ƒÆí Value Props', prompt: 'List our key value propositions and competitive differentiators' },
  { label: '=ƒÜÇ Go-to-Market', prompt: 'Outline a go-to-market strategy based on our business model and target market' },
  { label: 'GÜön+Å Competitive Analysis', prompt: 'Analyze our competitive landscape and positioning' },
  { label: '=ƒÆ¼ Messaging Framework', prompt: 'Create a messaging framework with tagline, positioning, and key messages' },
  { label: '=ƒôê Launch Timeline', prompt: 'Create a product launch timeline with milestones and activities' },
];

// Chart generation quick commands
const CHART_SUGGESTIONS = [
  { label: '=ƒôê Revenue Chart', prompt: 'Create a line chart showing our revenue trends over time using financial data' },
  { label: '=ƒÑº Expense Breakdown', prompt: 'Create a pie chart breaking down our expenses by category' },
  { label: '=ƒôè Sales Pipeline', prompt: 'Create a bar chart showing our customer pipeline by stage' },
  { label: '=ƒôë Growth Metrics', prompt: 'Create an area chart showing signups, customers, and revenue growth' },
];

const TONE_OPTIONS = [
  { id: 'professional', label: 'Professional', icon: '=ƒæö' },
  { id: 'persuasive', label: 'Persuasive', icon: '=ƒöÑ' },
  { id: 'casual', label: 'Casual', icon: '=ƒæï' },
  { id: 'technical', label: 'Technical', icon: 'GÜÖn+Å' },
];

const FORMAT_OPTIONS = [
  { id: 'auto', label: 'Auto', icon: 'G£¿' },
  { id: 'list', label: 'Bullet Points', icon: 'GÇó' },
  { id: 'table', label: 'Table', icon: 'Gûª' },
  { id: 'summary', label: 'Summary', icon: '=ƒô¥' },
];
    ...
    >
      ...
      <div className="px-4 py-2 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
        <div className="flex items-center gap-4">
            ...
            >
              <span className="text-lg">GÜÖn+Å</span> {showOptions ? 'Hide Options' : 'Show Options'}
            </button>
        ...
        >
            <span>=ƒîÉ</span> Web Search {isWebSearchEnabled ? 'ON' : 'OFF'}
        </button>
      </div>
        ...
        <div className="px-4 py-3 border-b border-gray-100 bg-white space-y-3">
          <div className="flex items-center justify-between text-[11px] font-semibold uppercase tracking-[0.35em] text-gray-400">
            ...
            {webSearchMode === 'images' && lastImageQuery && (
              <span className="tracking-normal text-[10px] text-gray-500">Last visuals: GÇ£{lastImageQuery}GÇ¥</span>
            )}
          </div>
          <div className="flex items-center gap-2">
            {[
              ...
              <div className="max-h-64 overflow-y-auto">
                {imageResults.length === 0 && !imageSearchError ? (
                  <div className="rounded-xl border border-dashed border-gray-300 px-4 py-6 text-center text-xs text-gray-500">
                    Describe the visual you need above, then tap GÇ£Fetch visualsGÇ¥ to preview research-grade images.
                  </div>
                ) : (
                  <div className="grid grid-cols-2 gap-2">
