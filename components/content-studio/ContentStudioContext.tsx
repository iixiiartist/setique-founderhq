...
export function ContentStudioProvider({ children }: ContentStudioProviderProps) {
  ...
  const createNewDocument = useCallback((title = 'Untitled Document', pageSize: string = '16:9') => {
    const sizeKey = (pageSize in PAGE_SIZES ? pageSize : '16:9') as PageSizeKey;
  ...
  const addPage = useCallback(() => {
    ...
    // Handle pageSize being either a key or an object with dimensions
    const pageSize = state.document.settings?.pageSize;
    let width = 1920;
    let height = 1080;
    ...
    } else if (typeof pageSize === 'string' && pageSize in PAGE_SIZES) {
      const sizeObj = PAGE_SIZES[pageSize as PageSizeKey];
      width = sizeObj.width;
  ...
  const deletePage = useCallback((index: number) => {
    ...
    const updatedPages = state.document.pages.filter((_: ContentPage, i: number) => i !== index);
    const updatedDocument: ContentDocument = {
      ...state.document,
      pages: updatedPages.map((p: ContentPage, i: number) => ({ ...p, order: i, name: `Page ${i + 1}` })),
      updatedAt: new Date().toISOString(),
    };
